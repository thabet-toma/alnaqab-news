...
- **M10 (النشر):** ✔ منشور على 3dd5abe. `/live-in/1` و`/live-in/2` ← 101، الصفحات 200، `config.php` ← 403، المحرّك يعمل وأوامر `visual.*` و`live*_gain` حيّة.
- **معلّق على المالك — صلاحية:** `usermod -aG www-data liquidsoap && systemctl restart liquidsoap`. بدونه دخول المتصفح بالرابط يُرفض (البوابة لا تقرأ `config.local.php`). BUTT والكتم والعرض تعمل بدونه.
- **معلّق على المالك — قبول بصوت حقيقي:** مذيع من المتصفح · ضيف من أندرويد مع TalkBack · كتم/طرد · قاعدة صوت الفيديو · OBS ← فيسبوك · Sharing Debugger · مكالمة Voicemeeter.
- **خارج النطاق (تذكرة منفصلة):** عنوان «الآن يُشغَّل» يحمل `\u0000` حرفياً من وسم الملف ويظهر على شاشة الاستوديو · `og:image` لصفحة الراديو صورة picsum مؤقتة من إعدادات الراديو.
